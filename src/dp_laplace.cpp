...
static void DPLaplaceNoiseFunction(DataChunk &args, ExpressionState &, Vector &result) {
	// value DOUBLE, epsilon DOUBLE, sensitivity DOUBLE
	auto &value_vec = args.data[0];
	auto &epsilon_vec = args.data[1];
	auto &sens_vec = args.data[2];

	UnifiedVectorFormat value_data, eps_data, sens_data;
	...
	for (idx_t i = 0; i < args.size(); i++) {
		...
		if (!value_data.validity.RowIsValid(vi) || !eps_data.validity.RowIsValid(ei) || !sens_data.validity.RowIsValid(si)) {
		...
		auto builder = differential_privacy::LaplaceMechanism::Builder();
		builder.SetEpsilon(epsilon);
		builder.SetL1Sensitivity(sensitivity);
